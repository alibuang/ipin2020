...
class mywindow(QMainWindow):
    ...
    def send_telegram_msg(self, master_comp, master_accName, slave_comp, slave_accName, symbol, gap_up, gap_down, comments):
        ...
        text_msg = 'timestamp: ' + timestamp + '\n' \
                   + 'broker A: ' + master_comp[:20] + '\n' \
                   + 'acct A: ' + master_accName[:20] + '\n' \
                   + 'broker B: ' + slave_comp[:20] + '\n' \
                   + 'acct B: ' + slave_accName[:20] + '\n' \
                   + 'symbol: ' + symbol + '\n' \
    ...
    def debug(self):
        ...
        magnums = master.get_magnums()
        symbols = master.get_symbols(magnums)

        for symbol in symbols:

            # print('symbol is {}'.format(symbol))
            last_open, server_time = master.get_lastopentime_bysymbol(symbol)
            print('{} : {}'.format(symbol, last_open))

        # magic_nums = master.get_magnums()
        #
        # for magic_num in magic_nums:
        #     last_open, server_time = master.get_opentime_bymagnum(magic_num)
        #     print('{} : {}'.format(magic_num, last_open))
    ...
    def opennew_arbitposition(self, master, slave):
        ...
        for i in range(len(self.cfg.symbols)):
            ...
            self.gap_up[i] = pow(10, digit) * (master.ask[i] - slave.bid[i])
            self.gap_down[i] = pow(10, digit) * (slave.ask[i] - master.bid[i])
            # print ('{} : gap up is {} and gap down is {}'.format(self.cfg.symbols[i], self.gap_up[i], self.gap_down[i]))

            # Open position management
            open_signal = self.get_open_signal(self.gap_up[i], self.gap_down[i], self.cfg.arb_open)
            # print('Open Signal for symbol {} is {}'.format(self.cfg.symbols[i],open_signal))

            if open_signal == DOWN and master.ask[i] != 0 and slave.ask[i] != 0 \
                    and self.chk_open_valid(master, master.symbols[i]):

                magic_number = self.generate_magicno()
                master.send_order2(order_type=SELL, symbol=master.symbols[i], price=master.bid[i] \
                                   , lot=self.cfg.master_lot, slip=self.cfg.master_slip, magic_number=magic_number \
                                   , comments=str(magic_number))
                slave.send_order2(order_type=BUY, symbol=slave.symbols[i], price=slave.ask[i], lot=self.cfg.slave_lot, \
                                  slip=self.cfg.slave_slip, magic_number=magic_number, comments=str(magic_number))

                self.send_telegram_msg(master.company, master.acctName, slave.company, slave.acctName \
                                       , self.cfg.symbols[i], self.gap_up[i], self.gap_down[i], 'DOWN')

            elif open_signal == UP and master.ask[i] != 0 and slave.ask[i] != 0 \
                    and self.chk_open_valid(master, master.symbols[i]):

                magic_number = self.generate_magicno()
                master.send_order2(order_type=BUY, symbol=master.symbols[i], price=master.ask[i] \
                                   , lot=self.cfg.master_lot, slip=self.cfg.master_slip, magic_number=magic_number \
                                   , comments=str(magic_number))
                slave.send_order2(order_type=SELL, symbol=slave.symbols[i], price=slave.bid[i], lot=self.cfg.slave_lot, \
                                  slip=self.cfg.slave_slip, magic_number=magic_number, comments=str(magic_number))
                ...
                self.send_telegram_msg(master.company, master.acctName, slave.company, slave.acctName \
                                       , self.cfg.symbols[i], self.gap_up[i], self.gap_down[i], 'UP')
    ...
    def chk_open_valid(self, broker, magic_number):
        open_valid = False

        openTime, mt4ServerTime = broker.get_lastopentime_bysymbol(magic_number)
