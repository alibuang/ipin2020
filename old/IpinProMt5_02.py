...
class mywindow(QMainWindow):
    ...
    def debug(self):
        ...
        # self.read_cfg_file()
        #
        # for i in range(0,len(self.cfg.symbols)):
        #     auto_trade.close_order(i)
        ...
        #     slave.get_acct_info()
        #
        #     symbol = slave.symbols[3]
        #     orderComment = self.cfg.slave_comment
        #     trade_count = slave.get_trade_count(symbol, orderComment)
        #     print("trade count is {}".format(trade_count))
        #
        #     print("Is next position ? '{}'".format(self.is_next_position(slave,symbol,UP,-130,-80)))

            # master.init_symbol()
            # slave.init_symbol()

            #     m_price = master.get_fx_price(symbol)
            #     print("Master Price: {}".format(m_price))
            #
            #     s_price = slave.get_fx_price(symbol)
            #     print("Slave Price: {}".format(s_price))
    ...
    def is_next_position(self, slave:Broker, symbol:str, signal:int, gap_up:float, gap_down:float):
        ...
        # print("pip step: {}".format(pip_step),
        #       "\ncomment: {}".format(comment),
        #       "\ntrade count: {}".format(trade_count),
        #       "\nnext step: {}".format(next_step))
